...
public class AddressServiceImpl implements IAddressService {
    ...
    public void updateAddress(Integer uid, String username, Address address) {
        ...
        // 调用addressMapper的update(Address address)方法更新收货地址数据，并获取返回的受影响行数
        Integer rows = addressMapper.update(address);
        // 判断受影响行数是否不为1
        if (rows != 1) {
            // 是：抛出InsertException
            throw new InsertException("更新收货地址数据时出现未知错误，请联系系统管理员！");
        }
    }
    ...
    @Override
    public Address getAid(Integer aid) {
        // 根据收货地址数据id，查询收货地址详情
        Address address1 = addressMapper.findByAid(aid);
        if (address1 == null) {
            throw new AddressNotFoundException("尝试访问的收货地址数据不存在");
        }
        address1.setProvinceCode(null);
        address1.setCityCode(null);
        address1.setAreaCode(null);
        address1.setCreatedUser(null);
        address1.setCreatedTime(null);
        address1.setModifiedUser(null);
        address1.setModifiedTime(null);
        return address1;
    }
}
