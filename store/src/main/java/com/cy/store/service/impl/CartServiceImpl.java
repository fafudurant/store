...
public class CartServiceImpl implements ICartService {
    ...
    public void deleteToCart(Integer cid) {
        // 根据参数cid，调用findByCid()查询购物车数据
        Cart result = cartMapper.findByCid(cid);
        // 判断查询结果是否为null
        if (result == null) {
            // 是：抛出CartNotFoundException
            throw new CartNotFoundException("尝试访问的购物车商品数据不存在");
        }

        // 根据参数cid，调用deleteById()执行删除
        Integer rows1 = cartMapper.deleteById(cid);
        if (rows1 != 1) {
    ...
    public Integer reduceNum(Integer cid, Integer uid, String username) {
        Cart result1 = cartMapper.findByCid(cid);
        if (result1 == null) {
            throw new CartNotFoundException("尝试访问的购物车数据不存在");
        }

        if (!result1.getUid().equals(uid)) {
            throw new AccessDeniedException("非法访问");
        }
        Integer num=0;
        if(result1.getNum()>2){
           num = result1.getNum() - 1;
        }
        // 创建当前时间对象，作为modifiedTime
        Date now = new Date();
        // 调用updateNumByCid(cid, num, modifiedUser, modifiedTime)执行修改数量
