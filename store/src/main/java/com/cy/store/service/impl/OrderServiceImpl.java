...
public class OrderServiceImpl implements IOrderService {
    ...
    public List<OrderVO> getByUid(Integer uid) {
        List<OrderVO> list = orderMapper.selectOrderByUid(uid);
        return list;
    }

    @Override
    public void deleteToOrder(Integer oid) {
        // 根据参数cid，调用findByCid()查询收货地址数据
        Order result = orderMapper.findByOid(oid);
        // 判断查询结果是否为null
        if (result == null) {
            // 是：抛出CartNotFoundException
            throw new OrderNotFoundException("尝试访问的购物车商品数据不存在");
        }

        // 根据参数cid，调用deleteById()执行删除
        Integer rows1 = orderMapper.deleteByOid(oid);
        if (rows1 != 1) {
            throw new DeleteException("删除购物车商品数据时出现未知错误，请联系系统管理员");
        }
    }
}
